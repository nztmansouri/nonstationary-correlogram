...
if nargout<2 & max(n1,n2)>100000
    
end

% preallocate using a rough estimate of # of time difference required (assuming independence)
maxTTT = (interval_max-interval_min);
eN = min(ceil((max(n1, n2))^2 * maxTTT * 2 / max(events1(end), events1(end))),10000000);
event_pairs = zeros(1 * eN, 3);
...
for n = 1:n1
    ...
    for m = lastStartIdx:n2
        ...
        if timeDiff >= interval_min
            ...
            if k>size(event_pairs,1)
                % block preallocate
                event_paris(k:(k+eN),:)=0;
            end
        end
    end
    if incIdx>0
